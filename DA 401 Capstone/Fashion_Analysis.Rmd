...
``` {r barplot color}
# Count colors as proportion of total
item_proportions <- Vogue_Images %>%
  group_by(year, season, designer, item_described) %>%
  summarise(count = n()) %>%
  group_by(year, designer) %>%
  mutate(proportion = count / sum(count))

# Create the bar plot
ggplot(item_proportions, aes(x= item_described, y = proportion)) + geom_bar(stat = "identity", position = "stack") +
  labs(x = "Clothing item", y = "Frequency", title = "Frequency of analyzed clothing items") + theme_minimal()
```

```{r color plots}
library(reshape2)
# Create color palette
colors <- c("black", "blue", "brown", "gray", "green", "beige", "orange", "pink", "purple", "red", "white", "yellow")
...
```
```{r color plot 2}
colors <- c("black", "blue", "brown", "gray", "green", "beige", "pink", "purple", "red", "white", "yellow", "orange")
...
```{r color plot 3}
colors <- c("black", "blue", "brown", "gray", "green", "beige", "orange", "pink", "red", "white", "yellow", "purple")
...
print(chisq1)
# Plot residuals
residuals <- as.data.frame(chisq1$residuals)
residuals <- as.data.frame(sapply(residuals, as.numeric))
barplot(t(residuals), beside = TRUE, legend.text = TRUE,
        main = "Residuals from Chi-Squared Test", 
...
print(chisq2)
# Plot residuals
residuals <- as.data.frame(chisq2$residuals)
residuals <- as.data.frame(sapply(residuals, as.numeric))
barplot(t(residuals), beside = TRUE, legend.text = TRUE,
        main = "Residuals from Chi-Squared Test", 
        xlab = "Category", ylab = "Residuals")
...
print(chisq3)
# Plot residuals
residuals <- as.data.frame(chisq3$residuals)
residuals <- as.data.frame(sapply(residuals, as.numeric))
barplot(t(residuals), beside = TRUE, legend.text = TRUE,
        main = "Residuals from Chi-Squared Test", 
        xlab = "Category", ylab = "Residuals")
